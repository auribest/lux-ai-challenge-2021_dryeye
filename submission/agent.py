    ...
    for unit in player.units:
        if unit.is_worker() and unit.can_act():
            if unit.get_cargo_space_left() > 0:
                closest_resource_tile = get_closest_resource_tile(unit=unit, resource_tiles=resource_tiles, player=player)
                ...
                if len(player.cities) > 0:
                    closest_city_tile = get_closest_city_tile(player=player, unit=unit)
