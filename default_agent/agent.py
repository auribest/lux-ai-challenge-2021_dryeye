...
# Initialize relevant dictionaries and lists
...
first_night_turns = list(np.arange(30, 351, 40))
...
def get_closest_workers(observation, unit, workers, amount=1):
    """
    Return a specific amount of closest workers for a specific unit.

    :param observation: (observation) Current game observation
    :param unit: (unit) Unit from which to get the closest workers from
    :param workers: (list) All workers
    :return: (list) The closest workers
    """
    ...
    for _ in range(amount):
        for worker in workers:
            dist = worker.pos.distance_to(unit.pos)
            if dist < shortest_dist and worker.id not in closest_workers:
                shortest_dist = dist
                closest_worker = worker

        if closest_worker is not None:
            closest_workers.append(closest_worker)

    if len(closest_workers) < amount:
        logging.info(f'{observation["step"]}: Amount of closest workers not fulfilled! Required amount = {amount} vs. Actual amount found = {len(closest_workers)}\n')

    return closest_workers
...
def is_enemy_city_tile_assigned(enemy_city_tile):
    """
    Check if an enemy city tile has been assigned to a unit.

    :param enemy_city_tile: (cell) The resource tile to check for assignment
    :return: (bool) Assigned or not assigned
    """
    # If the resource is already in the dictionary of assigned resources, return ture, else return false
    for already_assigned_enemy_city_tile in unit_to_resource_tile_dict.values():
        if already_assigned_enemy_city_tile is None:
            continue
        elif enemy_city_tile.pos.x == already_assigned_enemy_city_tile.pos.x and enemy_city_tile.pos.y == already_assigned_enemy_city_tile.pos.y:
            return True
...
def get_closest_unassigned_resource_tile(player, unit, resource_tiles):
    ...
    :return: (tuple of cell, float) The unassigned closest resource tile
    """
    shortest_dist = math.inf
    ...
    return closest_resource_tile, shortest_dist


def get_closest_resource_tile(player, unit, resource_tiles):
    """
    Get the closest resource tile.
...
def get_closest_unassigned_enemy_city_tile(unit, enemy_city_tiles):
    """
    Get the closest unassigned enemy city tile.

    :param unit: (unit) The unit from which to calculate the distance
    :param enemy_city_tiles: (list) All enemy city tiles on the map
    :return: (cell) The unassigned closest enemy city tile
    """
    shortest_dist = math.inf
    closest_enemy_city_tile = None

    # For every resource tile, check if it is a valid resource to gather, if it is unassigned, and if it the closest
    for enemy_city_tile in enemy_city_tiles:
        if is_enemy_city_tile_assigned(enemy_city_tile): continue

        dist = enemy_city_tile.pos.distance_to(unit.pos)
        if dist < shortest_dist:
            shortest_dist = dist
            closest_enemy_city_tile = enemy_city_tile

    return closest_enemy_city_tile
...
def get_closest_cluster(unit, clusters_of_resource_type, closest_cluster):
    ...
    for cluster_id in list(clusters_of_resource_type):
        ...
        if list(unit_to_cluster_id_dict.values()).count(cluster_id) < len(cluster) or not unit.is_worker():
...
def get_closest_city_and_resource(city_tiles, resource_cluster):
    ...
    :return: (tuple of cell, cell, float) The closest city tile, the closest resource tile, and their distance (the shortest distance)
    """
...
def get_adjacent_tile(game_state, observation, tile):
    """
    Get an adjacent tile for a given tile.

    :param game_state: (game) Current game state
    :param observation: (observation) Current game observation
    :param tile: (cell) The tile to check for adjacent tiles
    :return: (cell) The adjacent tile
    """
    adjacent_tile = None
    width, height = game_state.map.width, game_state.map.height

    # Check all adjacent tiles to see if they don't have a city on top of them
    for direction in adjacent_directions:
        coordinates = (tile.pos.x + direction[0], tile.pos.y + direction[1])
        ...
        potential_tile = game_state.map.get_cell(coordinates[0], coordinates[1])
        logging.info(f'{observation["step"]}: Checking potential tile ({potential_tile.pos.x}/{potential_tile.pos.y})\n')

        if potential_tile.road == 0 and potential_tile.citytile is None:
            adjacent_tile = potential_tile
            logging.info(f'{observation["step"]}: Found adjacent tile ({adjacent_tile.pos.x}/{adjacent_tile.pos.y})\n')
...
def tile_has_adjacent_city_tile(game_state, tile):
    ...
    for direction in adjacent_directions:
        ...
        if checking_tile.citytile is not None:
            has_adjacent_city_tile = True

    return has_adjacent_city_tile


def get_all_empty_adjacent_tiles_of_cluster(game_state, observation, cluster):
    """
    ...
    for resource_tile in cluster:
        empty_adjacent_tile = get_empty_adjacent_tile(game_state=game_state, observation=observation, tile=resource_tile)
...
def get_alternative_path(observation, a_star, city_tiles, closest_resource_tile):
    ...
    :return: (tuple of cell, list) New closest city tile and nodes that form a path
    """
    path = None
...
def get_alternative_tile_to_build_city(game_state, observation, a_star, unit, resource_tiles, city_tiles, resource_clusters, new_closest_city_tile=None):
    """
    ...
    closest_cluster = get_closest_cluster(unit=unit, clusters_of_resource_type=resource_clusters, closest_cluster=closest_cluster)
    ...
    # If the distance from the closest city to the closest resource tile of the cluster is not larger than one, build adjacent a city tile
        ...
        closest_city_tile_to_unit = get_closest_tile(unit=unit, tiles=city_tiles)
...
def get_tile_to_build_city(game_state, observation, a_star, unit, resource_tiles, city_tiles, resource_clusters, new_closest_city_tile=None):
    ...
    if empty_adjacent_tiles_with_adjacent_city is not None:
        ...
        tile_to_build_city = get_closest_tile(unit=unit, tiles=empty_adjacent_tiles)
    else:
        tile_to_build_city = get_alternative_tile_to_build_city(game_state=game_state, observation=observation, a_star=a_star, unit=unit, resource_tiles=resource_tiles, city_tiles=city_tiles, resource_clusters=resource_clusters, new_closest_city_tile=new_closest_city_tile)
...
def agent(observation, configuration):
    ...
    carts = [unit for unit in player.units if unit.is_worker() is not True]

    # logging.info(f'{observation["step"]}: Created a cart on city tile ({city_tile.pos.x}/{city_tile.pos.y})\n')
    ...
    ############ START CITY TILES ACTION TREE ############

    # Create a worker on every city tile possible, else research if possible
    can_create_worker = len(city_tiles) - len(units)
    cart_built = False
    if len(city_tiles) > 0:
        for city_tile in city_tiles:
            if city_tile.can_act():
                if can_create_worker > 0:
                    # Do not create carts for the moment
                    if player.researched_uranium() and len(carts) != 0 and len(list(uranium_clusters_dict.values())) > 0 and not cart_built:
    ...
    ############ END CITY TILES ACTION TREE ############

    ############ START WORKER ACTION TREE ############

    for unit in player.units:
        if unit.is_worker() and unit.can_act() and len(resource_tiles) > 0:
            # If cart exists, assign it to the closest worker unit
            # TODO: IMPLEMENT FUNCTIONALITY FOR MORE THAN ONE CART (NO DOUBLE WORKER ASSIGNMENT!)
            for cart in carts:
                # This has to be modified if multiple workers are to be assigned to a cart!
                [closest_worker] = get_closest_workers(observation=observation, unit=cart, workers=workers)
                worker_to_cart_dict[closest_worker.id] = cart

            # If cart exists and uranium is still available, assign it to the closest uranium resource tile
            for cart in list(worker_to_cart_dict.values()):
                # This has to be modified if multiple carts are to be assigned to a cluster!
                closest_uranium_cluster = get_closest_cluster(unit=cart, clusters_of_resource_type=uranium_clusters_dict, closest_cluster=None)
                if closest_uranium_cluster is not None:
                    closest_uranium_tile = get_closest_tile(unit=cart, tiles=closest_uranium_cluster)
                    unit_to_resource_tile_dict[cart.id] = closest_uranium_tile
                else:
                    logging.warning(f'{observation["step"]}: No uranium clusters found!\n')
            ...
            resource_tile_assignment, distance_to_resource = get_closest_unassigned_resource_tile(player=player, unit=unit, resource_tiles=resource_tiles)
            ...
            if unit.get_cargo_space_left() > 0:
                ...
                # If the assigned resource is None, get the closest unassigned enemy city tile and set as target destination, else destination is assigned resource
                if assigned_resource_tile is None:
                    enemy_city_target_tile = get_closest_unassigned_enemy_city_tile(unit=unit, enemy_city_tiles=enemy_city_tiles)
                    if enemy_city_target_tile is not None:
                        unit_to_target_tile_dict[unit.id] = enemy_city_target_tile
                        logging.info(f'{observation["step"]}: Worker {unit.id} wants to go to assigned enemy city tile ({enemy_city_target_tile.pos.x}/{enemy_city_target_tile.pos.y})')
                    else:
                        continue
                else:
            ...
            if len(cities) > 0:
                ...
                closest_city_tile = get_closest_tile(unit=unit, tiles=city_tiles)
                ...
                # If city fuel is enough to survive a certain amount of rounds, city will probably survive
                city_survives = 'ERROR'
                for start_night in first_night_turns:
                    if observation["step"] < start_night:
                        if observation["step"] >= start_night - 15:
                            if city_fuel <= city.get_light_upkeep() * 10:
                                city_survives = False
                            else:
                                city_survives = True
                ...
                if not city_survives and unit.get_cargo_space_left() <= 50:
                    ...
                    closest_city_tile = get_closest_tile(unit=unit, tiles=city_tiles_of_assigned_city)
            ...
            # If the unit has an assigned cart and the cart has an assigned resource, set the assigned uranium resource tile as destination for worker and cart
            if unit.id in worker_to_cart_dict:
                assigned_cart = worker_to_cart_dict[unit.id]
                if assigned_cart.id in unit_to_resource_tile_dict:
                    target_tile = unit_to_resource_tile_dict[assigned_cart.id]
                    adjacent_target_tile = get_adjacent_tile(game_state=game_state, observation=observation, tile=target_tile)
                    unit_to_target_tile_dict[unit.id] = target_tile
                    unit_to_target_tile_dict[assigned_cart.id] = adjacent_target_tile
    ...
    ############ START UNIT MOVEMENT ACTIONS ############
    # Unblock all city tiles as a fail-safe, in case they were blocked for some reason
    a_star.toggle_cities_to_blocking(game_state=game_state, set_blocking=False)

    for unit in units:
        # If the unit as no target tile, skip the unit and log a warning
        if unit.id not in unit_to_target_tile_dict or unit_to_target_tile_dict[unit.id] is None:
            logging.warning(f'{observation["step"]}: Unit {unit.id} has no target destination!!!\n')

            continue

        logging.info(f'{observation["step"]}: Unit {unit.id} wants to move to: ({unit_to_target_tile_dict[unit.id].pos.x}/{unit_to_target_tile_dict[unit.id].pos.y})\n')

        # If unit is not on target position, calculate next movement direction
        if unit.pos != unit_to_target_tile_dict[unit.id].pos:
            # Get start and end coordinates
            start_x, start_y = unit.pos.x, unit.pos.y
            end_x, end_y = unit_to_target_tile_dict[unit.id].pos.x, unit_to_target_tile_dict[unit.id].pos.y

            if unit.is_worker():
                # Block all city tiles if worker is a builder (so the carried resources are not accidentally dropped off)
                a_star.toggle_cities_to_blocking(game_state=game_state, set_blocking=is_builder[unit.id])

            # Calculate the path, if no path is found, skip the unit and log a warning
            path = a_star.find_path(start_x, start_y, end_x, end_y)
            if path is None:
                # TODO: Block not if he is on a city tile
                logging.warning(f'{observation["step"]}: No path for unit {unit.id} from ({start_x}/{start_y}) to ({end_x}/{end_y}) found!\n')
            ...
            logging.info(f'{observation["step"]}: Unit {unit.id} is moving in direction: {worker_direction}\n')

            # Append the movement action
            actions.append(unit.move(worker_direction))

            # Block the current target node for the next unit
            a_star.get_node(path[1].x, path[1].y).blocked = True
            worker_blocked_nodes.append(a_star.get_node(path[1].x, path[1].y))
        else:
            logging.info(f'{observation["step"]}: Unit {unit.id} is already on target position ({unit_to_target_tile_dict[unit.id].pos.x}/{unit_to_target_tile_dict[unit.id].pos.y}) \n')

    ############ END UNIT MOVEMENT ACTIONS ############
