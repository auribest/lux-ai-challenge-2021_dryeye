...
def train(args):
    ...
    list_of_models = glob.glob('models/*.zip')
    if len(list_of_models) == 0:
        opponent = Agent()
    else:
        latest_model = max(list_of_models, key=os.path.getctime)
        latest_model = latest_model.replace(".zip", "")
        latest_model = latest_model.replace("\\", "/")
        print(f'Opponent Path: {latest_model}')
        latest_model = PPO.load(latest_model)
        opponent = AgentPolicy(mode="inference", model=latest_model)
